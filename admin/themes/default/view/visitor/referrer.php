            ...
            <div class="content-page">
                ...
                <div class="content">
                    ...
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <h4 class="pull-left page-title">来源</h4>
                                <ol class="breadcrumb pull-right">
                                    <li><a href="#">Census</a></li>
                                    <li class="active">来源</li>
                                </ol>
                            </div>
                        </div>
                        <!-- /Page-Title -->
                            ...
                            <div class="col-lg-12">
                                <div class="panel panel-default panel-fill">
                                    ...
                                    <div class="panel-body"> 
                                        <table class="table  table-hover general-table" id="referrer_table">
                                            <thead>
                                                <tr>
                                                    <th width="60%">访客来源</th>
                                                    <th>访问量</th>
        ...
        <script>
            ...
            function get_data(t_start,t_end){
                $.post(_REQUEST_HOST+"/visitor/ajax_data",{'t_start':t_start,'t_end':t_end},function(e){
                    ...
                    var temp = {};
                    for(var i in e) {
                        var key= e[i].referrer;
                        if(temp[key]) {
                            temp[key].referrer = temp[key].referrer;
                            ...
                            temp[key] = {};
                            temp[key].referrer = e[i].referrer;
                    ...
                    for (var x in res) {
                        str += '<tr>';
                        str += '<td>'+res[x].referrer+'</td>';
                        str += '<td>'+res[x].nb_visits+'</td>';
                        str += '<td>'+formatSeconds(res[x].page_time)+'</td>';
                        str += '</tr>';
                        }
                    if(str!=''){
                        $('#referrer').empty().append(str);
                        $('#referrer_table').dataTable({
                            bRetrieve: true,
                            bDestroy: true,
                        });	
                    }else{
                        $('#referrer_table').dataTable().fnDestroy();
                        $('#referrer').empty().append('<tr><td colspan="3"><center>暂无数据</center></td></tr>');
                    }
                });
            }
            //初始页面自运行
